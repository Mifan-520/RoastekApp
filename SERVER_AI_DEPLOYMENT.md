...
2. `docs/2026-04-30-aliyun-deploy.md`（阿里云部署快速指南）
3. `docker-compose.yml`
4. `.env.example`
5. `frontend/nginx/default.conf`
6. `deploy/nginx/reverse-proxy.example.conf`
7. `frontend/src/app/routes.tsx`（前端路由入口）
8. `frontend/Report/ProductionReportPage.tsx`（总产量报表页）
9. `frontend/Report/reportData.ts`（报表演示数据）
...
- 设备 ID：`CQ-001`，类型：风机设备，通信方案：Modbus RTU 透传
- 单片机/变频器：Goodrive270，DTU：有人云 4G
- 种子数据已写入 `backend/src/data/devices.js`
- 前端设备详情页支持风机设备类型的运行频率、运行状态展示

### 新增文档

- `docs/2026-04-30-aliyun-deploy.md`：阿里云服务器快速部署指南
- `docs/2026-04-29-cq001-fan-device-design.md`：CQ-001 设备设计文档
...
| `LY-001` | 溧阳一期风机 | 活跃 | Goodrive270 变频器 | 有人云 4G | Modbus RTU 透传 |
| `CQ-001` | 重庆风机 | 活跃 | Goodrive270 变频器 | 有人云 4G | Modbus RTU 透传 |

### 历次同步累积要点

#### 前端

- 设备中心顶部统计区：运行设备、告警信息、数据报表三张卡片
- 报表页 `/report`：直线折线图，酒红色 `#940236` 主题，日/月/年切换
- 报表页固定演示数据：生豆 `1000kg`、烘焙 `800kg`、包装 `500kg`
- 风机设备详情页：运行频率（Hz）、运行状态展示

#### 后端

- 连接历史记录限制：`CONNECTION_HISTORY_LIMIT=30`
- MQTT 上线记录去重
- `trimConnectionHistory()` 自动裁剪
- LY-001、CQ-001 风机设备种子数据与 MQTT 处理
...
- 设备列表中 `LY-001`（溧阳一期风机）和 `CQ-001`（重庆风机）均出现
- 风机设备详情页可正常打开，显示运行频率、运行状态
- 设备列表卡片中的”活跃于”时间能跟随最新 telemetry 刷新
- 设备中心顶部”数据报表”卡片可跳转 `/report`
- `/report` 页面正常，日/月/年切换高亮为酒红色 `#940236`，折线图为直线
- 报表页三张数据卡片显示固定演示值：`1000kg / 800kg / 500kg`
- 停止发送 telemetry 超过离线阈值后，设备状态自动变为 `offline`
- 设备详情页上下线记录能正常展示
- 设备告警列表按时间降序，最新告警在最前

联调顺序：**确认服务器已同步 → 打开服务器页面 → 登录验证 → 调用 API 对比**。
