...
计划在C:\Users\Mifan\Desktop\app\docs下

### Codex 执行日志（必须）
Codex 执行时**必须**在项目根目录写 `.codex-log.md`，这是复盘的唯一入口。

格式（控制在 50 行以内）：
```markdown
## YYYY-MM-DD 任务标题
- 步骤1 [完成] 新增文件: backend/src/modbus-poller.js | 241行
- 步骤2 [完成] 修改文件: backend/src/mqtt-handler.js, backend/src/data/devices.js
- 步骤3 [失败] 原因: test timeout | 修改文件: backend/test/auth.test.js
- 步骤4 [跳过] 原因: 前置步骤失败
- 总计: +774 -44 行 | 14个文件 | 25个测试通过
```

每步必须包含：状态、文件列表、行数变化。[完成]的步骤只列文件不展开 diff。

### Claude Code 复盘
1. **只读 `.codex-log.md`**（~50行），不全量 `git diff`
2. 对 `[失败]` 步骤 → `git diff -- <具体文件>` 定向查看
3. 对 `[完成]` 步骤 → 抽查关键文件，不逐行审
4. 跑测试验证：`node --test test/`
5. 确认后删除 `.codex-log.md`
