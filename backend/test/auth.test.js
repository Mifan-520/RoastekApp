...
test("seeds thirteen devices with six pre-bound devices", async () => {
  const app = await buildApp();
  const session = await loginAs(app, "admin", "admin");

  assert.equal(seedDevices.length, 13);
  ...
  assert.equal(response.status, 200);
  assert.equal(response.body.devices.length, 6);
  assert.deepEqual(
    response.body.devices.map((device) => device.name),
    ["Z字梯", "生豆处理站", "智能仓储", "三元催化", "溧阳一期风机", "重庆风机"]
  );
